...
def sort_by_xycut(
    ...
    if axis in pre_cuts:
        ...
        if direction == 1:
            _recursive_yx_cut(
                block_bboxes, np.arange(len(block_bboxes)).tolist(), res, min_gap
            )
        else:
            _recursive_xy_cut(
                block_bboxes, np.arange(len(block_bboxes)).tolist(), res, min_gap
...
def _get_sub_category(
    ...
    relevant_blocks = [
        block
        for block in blocks
        if block["block_label"] in sub_title_labels + vision_labels
    ]
    region_bbox = None
    if relevant_blocks:
    ...
    for i, block1 in enumerate(blocks):
        ...
        # Condition 1: Centered check (must be within ±20 in both horizontal and vertical directions)
        is_centered = (
            abs(block_x_center - region_x_center) <= 20
            and abs(block_y_center - region_y_center) <= 20
        )

        # Condition 2: Length is greater than half of the layout region
        if is_horizontal_1:
            block_length = x2 - x1
