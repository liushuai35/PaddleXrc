...
def paddlex_generate_input_data(data, gt_data):
    ...
    input_data = {
        ...
        'sub_indices': [],
        'sub_bboxes': [] ,
        'page_scale': [gt_block_size[0] / block_size[0], gt_block_size[1] / block_size[1]]
    }
    for sub_block in parsing_result:
        if sub_block.get("index") != None:
            input_data["sub_bboxes"].append(list(map(int, np.array(sub_block["block_bbox"]) * np.array(input_data['page_scale'] * 2))))
...
if __name__ == '__main__':
    ...
    # gt_data = load_data_from_json("/mnt/shuailiu35/compare_with_mineru/70/gt_70.json") # block bbox太大，bleu score可能不是一个很好的指标
    gt_data = load_data_from_json("/mnt/shuailiu35/compare_with_mineru/30/gt_30.json") # block bbox太大，bleu score可能不是一个很好的指标
    ...
    # input_jsons = glob.glob("/mnt/shuailiu35/PaddleX/api_examples/pipelines/output1/*.json")#70
