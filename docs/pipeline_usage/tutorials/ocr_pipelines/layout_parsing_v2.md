...
markdown_texts,markdown_images = pipeline.concatenate_markdown_pages(
                            layout_parsing_pipeline_output=output)
...
<td><code>layout_parsing_pipeline_output</code></td>
<td><code>Iterator</code></td>
<td>包含每一页Markdown数据的迭代器对象</td>
...
- 调用 `concatenate_markdown_pages()` 方法将 `layout parsing pipeline` 输出的多页Markdown内容`layout_parsing_pipeline_output`合并为单个完整文档，并返回合并后的Markdown内容以及图像列表。
